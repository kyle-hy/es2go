...
// PreAggRangeHistStatsCond 使用go代码预处理渲染需要的一些逻辑，template脚本调试困难
func PreAggRangeHistStatsCond(mappingPath string, esInfo *EsModelInfo, stype string) []*FuncTplData {
	...
	for _, condCmb := range mustFileds {
		...
		for _, histCmb := range histCmbs {
			for _, statsCmb := range statsCmbs {
				names := getAggRangeHistStatsFuncName(esInfo.StructName, condCmb, histCmb, statsCmb, rangeTypes, genCfg.CmpOptList, stype)
				comments := getAggRangeHistStatsFuncComment(esInfo.StructComment, condCmb, histCmb, statsCmb, rangeTypes, genCfg.CmpOptList, stype)
				params := getAggRangeHistStatsFuncParams(condCmb, rangeTypes, genCfg.CmpOptList)
				queries := getAggRangeHistStatsQuery(condCmb, histCmb, statsCmb, rangeTypes, genCfg.TermInShould, genCfg.CmpOptList, stype)
...
// getAggRangeHistStatsFuncName 获取函数名称
func getAggRangeHistStatsFuncName(structName string, condFields, histFields, statsFields []*FieldInfo, rangeTypes []string, optList [][]string, stype string) []string {
	if len(optList) == 0 {
		optList = CmpOptList
	}
	...
	names := []string{}
	fn := stype + GenFieldsName(statsFields) + "InHist" + GenFieldsName(histFields) + "Of" + structName + "By"
	// 多字段之间的两两组合
...
// getAggRangeHistStatsFuncComment 获取函数注释
func getAggRangeHistStatsFuncComment(structComment string, condFields, histFields, statsFields []*FieldInfo, rangeTypes []string, optList [][]string, stype string) []string {
	if len(optList) == 0 {
		optList = CmpOptList
	}
	...
	for _, fopt := range fopts {
		...
		fcmt := otherComment + fopt + "检索" + structComment + "，并按" + GenFieldsCmt(histFields, true) + "区间分桶统计" + GenFieldsCmt(statsFields, true) + "的" + HistStatNames[stype] + "\n"
...
// getAggRangeHistStatsFuncParams 获取函数参数列表
func getAggRangeHistStatsFuncParams(fields []*FieldInfo, rangeTypes []string, optList [][]string) []string {
...
// getAggRangeHistStatsQuery 获取函数的查找条件
func getAggRangeHistStatsQuery(condFields, histFields, statsFields []*FieldInfo, rangeTypes []string, termInShould bool, optList [][]string, stype string) []string {
	types, other := FieldFilterByTypes(condFields, rangeTypes)
	// match部分参数
...
// GenEsAggRangeHistStats 生成es检索详情
func GenEsAggRangeHistStats(mappingPath, outputPath string, esInfo *EsModelInfo) error {
	for _, stype := range HistStatsTypes {
		// 预处理渲染所需的内容
		funcData := PreAggRangeHistStatsCond(mappingPath, esInfo, stype)
		...
		// 写入文件
		suffix := fmt.Sprintf("_agg_range_hist_%s.go", strings.ToLower(stype))
