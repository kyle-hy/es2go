...
// PreAggRangeTermsCond 使用go代码预处理渲染需要的一些逻辑，template脚本调试困难
func PreAggRangeTermsCond(mappingPath string, esInfo *EsModelInfo) []*FuncTplData {
	...
	for _, cfs := range mustFileds {
		...
		for _, tcmb := range termsCmbs {
			names := getAggRangeTermsFuncName(esInfo.StructName, cfs, tcmb, rangeTypes, genCfg.CmpOptList)
			comments := getAggRangeTermsFuncComment(esInfo.StructComment, cfs, tcmb, rangeTypes, genCfg.CmpOptList)
			params := getAggRangeTermsFuncParams(cfs, rangeTypes, genCfg.CmpOptList)
			queries := getAggRangeTermsQuery(cfs, tcmb, rangeTypes, genCfg.TermInShould, genCfg.CmpOptList)
...
// getAggRangeTermsFuncName 获取函数名称
func getAggRangeTermsFuncName(structName string, fields, termsFields []*FieldInfo, rangeTypes []string, optList [][]string) []string {
	if len(optList) == 0 {
		optList = CmpOptList
	}
...
// getAggRangeTermsFuncComment 获取函数注释
func getAggRangeTermsFuncComment(structComment string, fields, termsFields []*FieldInfo, rangeTypes []string, optList [][]string) []string {
	if len(optList) == 0 {
		optList = CmpOptList
	}
...
// getAggRangeTermsFuncParams 获取函数参数列表
func getAggRangeTermsFuncParams(fields []*FieldInfo, rangeTypes []string, optList [][]string) []string {
	if len(optList) == 0 {
		optList = CmpOptList
	}
...
// GenEsAggRangeTerms 生成es检索详情
func GenEsAggRangeTerms(mappingPath, outputPath string, esInfo *EsModelInfo) error {
	// 预处理渲染所需的内容
