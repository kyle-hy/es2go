	...
	for _, condCmb := range fieldCmds {
		...
		for _, histCmb := range histCmbs {
			for _, statsCmb := range statsCmbs {
				names := getAggMatchHistStatsFuncName(esInfo.StructName, condCmb, histCmb, statsCmb)
				comments := getAggMatchHistStatsFuncComment(esInfo.StructComment, condCmb, histCmb, statsCmb)
				queries := getAggMatchHistStatsQuery(condCmb, histCmb, statsCmb)
...
func getAggMatchHistStatsFuncName(structName string, condFields, histFields, statsFields []*FieldInfo) []string {
	names := []string{}
...
func getAggMatchHistStatsFuncComment(structComment string, condFields, histFields, statsFields []*FieldInfo) []string {
	cmts := []string{}
	for _, stype := range HistStatsTypes {
		// 函数注释
...
func getAggMatchHistStatsQuery(condFields, histFields, statsFields []*FieldInfo) []string {
	queries := []string{}
	for _, stype := range HistStatsTypes {
		// match部分参数
...
func GenEsAggMatchHistStats(mappingPath, outputPath string, esInfo *EsModelInfo) error {
	...
	// 写入文件
	suffix := fmt.Sprintf("_agg_match_hist_stats.go")
